    ...
    return (
        ...
        >
            ...
            {testimonials.slice(0, imagePositions.length).map((testimonial, index) => (
                <motion.div
                    key={index}
                    className={cn('absolute rounded-lg shadow-xl cursor-pointer group', imagePositions[index].className)}
                    style={{
                        top: imagePositions[index].top,
                        left: imagePositions[index].left,
                        right: imagePositions[index].right,
                        bottom: imagePositions[index].bottom,
                        zIndex: hoveredIndex === index ? 60 : 10
                    }}
                ...
                >
                    <motion.img
                        ...
                        alt={testimonial.alt}
                        className="w-full h-full object-cover rounded-lg border-2 border-transparent group-hover:border-white/50 transition-colors"
                        animate={hoveredIndex === index ? {} : floatingAnimation()}
                    />

                    {/* Tooltip Card */}
                    <div className={cn(
                        "absolute left-1/2 -translate-x-1/2 bottom-full mb-3 w-64 p-4 rounded-xl bg-black/90 border border-white/10 text-left shadow-2xl backdrop-blur-md pointer-events-none transition-all duration-300 origin-bottom",
                        hoveredIndex === index ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-2"
                    )}>
                        <div className="text-white font-bold text-sm mb-0.5">{testimonial.name}</div>
                        <div className="text-white/50 text-xs mb-2 uppercase tracking-wide">{testimonial.role}</div>
                        <p className="text-white/80 text-xs italic leading-relaxed">"{testimonial.quote}"</p>
